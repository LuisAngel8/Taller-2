...
if (Entrada == 1){
  ...
  if (Entrada == 2){
    ...
    if (TExp == 1){
      print("\nCoeficiente de correlacion de Pearson del experimento A\n")
      pearson = cor(x = Fl$Sepal.Length, y = Fl$Sepal.Width, method = c("pearson"))
      print(pearson)
      
      print("\nCoeficiente de correlacion de Spearman del experimento A\n")
      ...
      print("\nCoeficiente de correlacion de Spearman del experimento B\n")
      spearman2 = cor(x = F2$Petal.Length, y = F2$Petal.Width, method = c("spearman"))
      print(spearman2)
    }
  } else if(Entrada == 4){
    print("Diagrama de dispersión + linea de regresión lineal\n", quote = F)
    print("1. Experimento_A", quote = F)
    print("2. Experimento_B", quote = F)
    TExp = as.integer(readline("Seleccione un experimento: "))
