...
## Graficar función de densidad distribucion de Bernoulli (binomial)

dbinom(x, size, prob, log)

-   **x =** Valores del eje X

-   **size =** Tamaño o numero de ensayos
...
#Se realiza la funcion dbinom() correspondiente a la funcion de densidad de una distribucion de Bernoulli (Los valores de size y prob deben ser positivos para evitar errores en los calculos de la funcion).

b= dbinom(x= Vec1, size = 50, prob = 0.4)
b
...
plot(b, type="h",lwd="2", xlab="Número de exitos", ylab="Probabilidad P(X=x)", col="pink")
```
